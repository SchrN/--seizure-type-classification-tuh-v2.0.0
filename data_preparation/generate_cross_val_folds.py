...
if __name__ == '__main__':
    ...
    elif tuh_eeg_szr_ver == 'v2.0.0': # for v1.5.2
        print('\nGenerating seizure wise cross validation for', tuh_eeg_szr_ver)
        fold_num = 5
        save_data_dir = os.path.join(args.save_data_dir, 'v2.0.0', 'raw_seizures')
        cv_random = generate_seizure_wise_cv_folds(save_data_dir, fold_num)
        pickle.dump(cv_random, open('cv_split_5_fold_seizure_wise_v2.0.0.pkl', 'wb'))
        """
        print('\nGenerating seizure wise cross validation for', tuh_eeg_szr_ver)
        fold_num = 3
        save_data_dir = os.path.join(args.save_data_dir, '2.0.0', 'raw_seizures')
        cv_patient_wise = generate_patient_wise_cv_folds(save_data_dir, fold_num)
        pickle.dump(cv_patient_wise, open('cv_split_3_fold_patient_wise_v2.0.0.pkl', 'wb'))
        """
    else:
